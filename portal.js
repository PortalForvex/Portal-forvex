...
function abrirFicha(){
  var nif = cu ? cu.nif : '';
  window.open('https://portalforvex.github.io/Ficha-Colaborador-Forvex/?nif=' + nif, '_blank');
}
...
async function loadAColab(){
  ...
  el.querySelectorAll('.btn-completar-ficha').forEach(function(btn){
    btn.addEventListener('click',function(){
      ...
      window.open('https://portalforvex.github.io/Ficha-Colaborador-Forvex/?adm=1&nif='+colab.nif,'_blank');
    });
  });
}
...
async function loadDashAdm(){
  ...
  const fichasHTML=(fichasPendentes||[]).map(c=>`
    <div style="display:flex;align-items:center;justify-content:space-between;padding:10px 14px;border-bottom:0.5px solid #E8C97A;flex-wrap:wrap;gap:8px">
      ...
      <button onclick="window.open('https://portalforvex.github.io/Ficha-Colaborador-Forvex/?adm=1&nif=${c.nif}','_blank')" style="font-size:12px;padding:5px 12px;border-radius:8px;border:none;background:#BA7517;color:#fff;cursor:pointer">Completar ficha</button>
    </div>`).join('');
